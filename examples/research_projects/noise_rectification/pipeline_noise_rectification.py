...
class NoiseRectificationPipeline(DiffusionPipeline):
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if noise_rectification_period is not None:
                    ...
                    if (
                        i >= len(timesteps) * noise_rectification_period[0]
                        and i < len(timesteps) * noise_rectification_period[1]
                    ):
                        delta_frames = noise - noise_pred
                        delta_noise_adjust = (
                            noise_rectification_weight
                            * (delta_frames[:, :, [0], :, :].repeat((1, 1, video_length, 1, 1)))
